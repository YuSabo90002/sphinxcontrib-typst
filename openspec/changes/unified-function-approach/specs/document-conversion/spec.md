...
### Requirement: Import Zoneの扱い

ドキュメントの先頭にあるimportゾーン（`#import`, `#show`）は**コードブロックの外**に配置しなければならない (MUST)。コードブロックはimportゾーンの後から開始しなければならない (MUST)。

**Rationale**: Import statements (`#import`) and template application (`#show`) operate at the top level of Typst documents and cannot be nested inside code blocks. These must remain at document top level with `#` prefix. The content code block starts after the import zone.

#### Scenario: Import zoneとcontent zoneの分離

```gherkin
GIVEN a Sphinx document that requires mitex and template
WHEN the builder generates the complete Typst document
...
AND import zone MUST be outside the code block
AND content zone MUST be inside the code block
```

#### Scenario: Import zone generated by builder

```gherkin
GIVEN the builder writes a document with template
WHEN the builder generates imports and translator generates content
THEN the builder MUST generate import zone at top
AND the translator MUST wrap content in #{...}
AND import zone MUST come before code block
```
...
### Import Zone (Builder responsibility)

The builder generates the import zone BEFORE translator starts:

```python
# In builder.py - write_doc() or similar
def write_doc(self, docname, doctree):
    # Generate import zone
    self.write_imports()  # Writes #import statements
    self.write_template_application()  # Writes #show: project.with(...)

    # Then translator generates content wrapped in #{...}
    destination = StringOutput(encoding="utf-8")
    self.current_docname = docname  # Track for relative paths
    doctree.walkabout(TypstTranslator(self, doctree))
```

**Key point**: Import zone is written by builder BEFORE translator runs.

### Document Wrapper (Translator responsibility)

Wrap entire document content in code mode:
...
**Result structure**:
```typst
// Builder generates this
#import ...
#show: project.with(...)

// Translator generates this
#{
  heading(...)
  par(...)
}
